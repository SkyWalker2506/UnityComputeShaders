...
public class GPUPhysics : MonoBehaviour {
	...
	void InitParticles()
    {
		...
		Vector3 offset = Vector3.one * centerer;
		for (int x = 0; x < particlesPerEdge; x++)
		{
			for (int y = 0; y < particlesPerEdge; y++)
			{
				...
				{
					Vector3 pos = new Vector3(x, y, z) * particleDiameter + offset;
	...
	void InitBuffers()
    {
		...
		int numberOfParticles = rigidBodyCount * particlesPerBody;
		particlesBuffer = new ComputeBuffer(numberOfParticles, SIZE_PARTICLE);
	...
	void InitShader()
	{
		...
		groupsPerRigidBody = Mathf.CeilToInt(particlesPerBody / 8f);
	...
	void Update() {
		...
		{
			shader.Dispatch(kernelGenerateParticleValues, groupsPerRigidBody, rigidBodyCount, 1);
